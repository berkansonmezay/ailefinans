...
export const CalendarScreen = ({ navigation }: any) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await fetchApi<any>('/calendar');
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.title || item.name || 'İsimsiz Etkinlik';
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Yaklaşan bir etkinlik bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
