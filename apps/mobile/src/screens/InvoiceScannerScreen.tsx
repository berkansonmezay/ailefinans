...
export const InvoiceScannerScreen = ({ navigation }: any) => {
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Fatura Tarayıcı yüklenirken hata:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Fatura Tarayıcı</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.merchantName || item.title || 'İsimsiz Fatura';
            const itemSubtitle = item.date ? new Date(item.date).toLocaleDateString('tr-TR') : 'Tarih Yok';
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Henüz bir fatura bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
