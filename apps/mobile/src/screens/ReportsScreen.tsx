    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.title || item.month || 'İsimsiz Rapor';
            const itemSubtitle = item.description || 'Detay Yok';
            const itemAmount = item.total || item.amount || 0;
            
            return (
              <View style={styles.card}>
                <View style={styles.cardIcon}>
                  <Ionicons name="pie-chart" size={28} color="#a855f7" />
                </View>
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Henüz bir rapor bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
