...
export const SavingsScreen = ({ navigation }: any) => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await fetchApi<any>('/savings');
      const dataList = Array.isArray(res) ? res : (res.items || res.data || []);
      setData(dataList);
    } catch (error) {
      console.error('Birikimlerim yüklenirken hata:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.name || item.goalName || 'İsimsiz Birikim';
            const itemSubtitle = item.status || 'Aktif';
            const itemAmount = item.currentAmount || item.amount || 0;
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Henüz bir birikim bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
