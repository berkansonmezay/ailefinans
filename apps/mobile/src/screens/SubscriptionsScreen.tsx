    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.name || item.platform || 'İsimsiz Abonelik';
            const itemSubtitle = item.billingCycle || 'Aylık';
            const itemAmount = item.price || item.amount || 0;
            
            return (
              <View style={styles.card}>
                <View style={styles.cardIcon}>
                  <Ionicons name="sync-circle" size={28} color="#8b5cf6" />
                </View>
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Henüz bir abonelik bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
