...
export const WarrantiesScreen = ({ navigation }: any) => {
  ...
  const loadData = async () => {
    try {
      ...
      console.error('Garantiler & Fişler yüklenirken hata:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Garantiler & Fişler</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const itemTitle = item.productName || item.title || 'İsimsiz Ürün';
            const itemSubtitle = item.expiryDate ? `Bitiş: ${new Date(item.expiryDate).toLocaleDateString('tr-TR')}` : 'Tarih Yok';
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>Henüz bir garanti belgesi bulunmuyor.</Text>
          }
        />
      )}
    </SafeAreaView>
