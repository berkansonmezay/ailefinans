...
export function CryptoTransactionsModal({ isOpen, onClose, symbol }: CryptoTransactionsModalProps) {
  ...
  const loadTransactions = async () => {
    try {
      setLoading(true);
      const data = await fetchApi<CryptoTransaction[]>(`/crypto/${symbol}/transactions`);
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${symbol} İşlem Geçmişi`}>
