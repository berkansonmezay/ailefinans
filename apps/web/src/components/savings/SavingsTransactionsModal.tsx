...
export function SavingsTransactionsModal({ isOpen, onClose, asset }: SavingsTransactionsModalProps) {
  if (!asset) return null;
  
  // asset.transactions is fetched along with the asset in GET /savings-assets
  const transactions: SavingsTransaction[] = asset.transactions || [];

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`${asset.code} İşlem Geçmişi`}>
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-text-primary">
                      {tx.quantity} Miktar @ {formatCurrency(tx.price)}
                    </p>
