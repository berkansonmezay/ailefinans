...
export function StockBuyModal({ isOpen, onClose, onSuccess, initialSymbol = '' }: StockBuyModalProps) {
  ...
  const handleSelectStock = async (selectedSymbol: string) => {
    ...
    try {
      const quote = await fetchApi<any>(`/stocks/${finalSymbol}/quote`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Hisse alımı başarıyla kaydedildi.');
      onSuccess();
      onClose();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Hisse Alımı Ekle">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative" ref={dropdownRef}>
          <label className="block text-sm font-medium text-text-muted mb-1">
            Hisse Kodu (Örn: THYAO) <span className="text-danger">*</span>
          </label>
          <input
            type="text"
            value={searchQuery}
              ...
              {isSearching ? (
                ...
                <ul className="py-1">
                  {searchResults.map((result, idx) => (
                    ...
                    >
                      <span className="font-bold text-text-primary">{result.symbol.replace('.IS', '')}</span>
